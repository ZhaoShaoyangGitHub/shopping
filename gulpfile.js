...
function jsFun(allFile, isProd) {
  var b = gulp.src([config.srcPath + config.jsPath + '**/*.js']);
  if (!allFile) {
    b = b.pipe(gulpPlugins.changed(config.devPath, {extension: '.js'}));
  }
  b = b.pipe(babel({
      presets: ['@babel/env']
    }))
    .pipe(gulp.dest((isProd ? config.buildPath : config.devPath) + 'js'));
  return isProd ? b : b.pipe(reload({stream: true}));
}
 ...
 * 自动刷新live-reloading
 */
gulp.task('devServer', ['devPugPublic', 'devScssPublic', 'devJsPublic', 'devCopy'], function () {
  browserSync({
    notify: false,
    open: options.o,
    port: 3000,
  ...
  gulp.watch([config.srcPath + config.jsPath + '**/*.js'], ['devJs'], reload);
...
gulp.task('devJsPublic', function () {
  return jsFun(true);
});
